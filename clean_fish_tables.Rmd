...
iepfishcodes <- readxl::read_excel("Data/IEPFishCodes.xlsx", sheet = 2) %>% as.data.frame() %>%
...
fish_ref_clean <- fish_ref %>% 
  ...
  select(EventID, StationCode, Datetime, SampleDate, WaterTemp, SpecificConductance, Conductivity, Turbidity, DO, pH, Secchi, WeatherCode, VegetationRank, MicrocystisRank, MethodCode, Gear, GearConditionCode) %>%
  arrange(Datetime)
...
smelt_gen_clean <- smelt_gen %>%
  ...
  select(FishTagID, qDSM, qWAG, qLFS, GeneticID) 
...
salm_gen_clean <- salm_gen %>%
  ...
  select(FishTagID, GeneticID, Probability1 = Prob1, GeneticID2, Probability2 = Prob2, SalmComments)
...
Sample_effort_clean <- Sample_effort_seine %>%
  ...
  mutate(SamplingAltered = ifelse(EventID %in% sampleChange, 1, 0))
```
...
Sample_alone <- Sample_effort_clean %>%
  select(EventID, StationCode, Datetime, SampleDate, WaterTemp, SpecificConductance, Conductivity, Turbidity, DO, pH, Secchi, Tide, WeatherCode, VegetationRank,SubstrateCode, HabitatType, MicrocystisRank, MethodCode, Gear, GearConditionCode, SamplingAltered,  FieldComments)
```
...
ggplot(fish_WQ_outliers3) + geom_point(aes(pH, WaterTemp, color = Region))
```
...
```{r}
fish_sample_cleaned <- left_join(Outliers_wide, Sample_alone) %>%
 select(EventID, StationCode, Datetime, SampleDate, WaterTemp, SpecificConductance, Conductivity, Turbidity, DO, pH, Secchi, Tide, WeatherCode, VegetationRank,SubstrateCode, HabitatType, MicrocystisRank, MethodCode, Gear, GearConditionCode, SamplingAltered,  FieldComments, Flag_WQ, Comment_WQ) %>%
...
Just an overall look at catch and length - leaving these as they are.
```{r}
...
check_counts <- fish_catch_QC %>%
  ...
  select(EventID, Datetime, StationCode, SampleID, OrganismCode, ForkLength, Count, PlusCount)
...
Count1 = c(79900, 79799, 113633)
# These are samples that are helping account for others that combined plus count and a measured fish into one row
Count2 = c(110230, 113643, 146681)
# These are samples that should be a plus count only and not also a fork length
FLNA = c(110219, 113624, 113645, 146676)
...
# Check STTD times - seem 7 hours off


# 110219 needs another row - one for the plus count, one for the individual. For now I have made the count 2 for another individual with FL 83 (accurate)
# - Same for 113645 and 146676

# 113624 needs another row - one for the individual. I made the existing one just for plus count. Should be ForkLength 28 and Count 1, Plus count FALSE
```
...
```{r}
write_csv(stations_clean, "clean_tables/stations.csv")
write_csv(fish_sample_cleaned, "clean_tables/event.csv")
write_csv(effort_final, "clean_tables/effort.csv")
write_csv(total_catch, "clean_tables/total_catch.csv")
write_csv(individuals, "clean_tables/fish_unique")
write_csv(taxonomy_clean, "clean_tables/taxonomy.csv")
write_csv(salm_gen_clean, "clean_tables/genetics_salmon.csv")
write_csv(smelt_gen_clean, "clean_tables/genetics_smelt.csv")
```
