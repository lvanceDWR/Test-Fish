...
# Fish catch data retrieved 8/5/2021, others 7/20/2021
...
fish_ref_clean <- fish_ref %>% 
  ...
  select(EventID, StationCode, Datetime, SampleDate, WaterTemp, SpecificConductance, Conductivity, Turbidity, DO, pH, Secchi, WeatherCode, VegetationRank, MicrocystisRank, MethodCode, Gear, GearConditionCode, SampleAltered) %>%
  arrange(Datetime)
...
# Make OrganismID
fish_catch_clean <- left_join(fish_catch_clean, fish_ref_clean %>% select(EventID, Datetime, StationCode, MethodCode)) %>%
  arrange(Datetime, MethodCode, StationCode) %>%
  group_by(Datetime, StationCode, OrganismCode) %>%
  mutate(num = 1:n()) %>%
  ungroup() %>%
  mutate(SampleID = paste0(format(Datetime,"%Y%m%d_%H%M"), "_", StationCode, "_", OrganismCode, num)) %>%
  select(-num, -Datetime, StationCode, MethodCode)
...
smelt_gen_clean <- smelt_gen %>%
  dplyr::rename(GeneticID = SPECIES..Assay.ID.,
                qDSM = q..DS.,
                qLFS = q..LF.,
                qWAG = q..WK.,
                NumChrom = X..Chroms,
  ...
  select(FishTagID, qDSM, qWAG, qLFS, GeneticID, Comments_Smelt) 
...
salm_gen_clean <- salm_gen %>%
  ...
  select(FishTagID, GeneticID, Probability1 = Prob1, GeneticID2, Probability2 = Prob2, Comments_Salmon, Comments_SalmonDataUsage)
...
Sample_effort_seine <- left_join(Sample_effort, seine_clean) %>%
...
Sample_alone <- Sample_effort_clean %>%
  select(EventID, StationCode, Datetime, SampleDate, WaterTemp, SpecificConductance, Conductivity, Turbidity, DO, pH, Secchi, Tide, WeatherCode, VegetationRank,SubstrateCode, HabitatType, MicrocystisRank, MethodCode, Gear, GearConditionCode, SampleAltered,  FieldComments)
```
...
fish_sample_cleaned <- left_join(Sample_alone, Outliers_wide %>% select(EventID, Flag_WQ, Comment_WQ)) %>%
 select(EventID, StationCode, Datetime, SampleDate, WaterTemp, SpecificConductance, Conductivity, Turbidity, DO, pH, Secchi, Tide, WeatherCode, VegetationRank,SubstrateCode, HabitatType, MicrocystisRank, MethodCode, Gear, GearConditionCode, SampleAltered,  FieldComments, Flag_WQ, Comment_WQ) %>%
...
Join with MethodCode
There are a number of missing methods. Some of these are in the original fish_ref table... check some out at some point. Hard to figure out with no date.
```{r}
...
Just an overall look at catch and length - leaving these as they are since the extra high lengths, counts seem plausible
```{r}
ggplot(fish_catch_QC) + geom_boxplot(aes(x = OrganismCode, y = ForkLength, fill = OrganismCode)) + 
  facet_wrap(~MethodCode, scales = "free",nrow = 3) + theme(axis.text.x = element_text(angle = 90))

ggplot(fish_catch_QC) + geom_boxplot(aes(x = OrganismCode, y = Count, fill = OrganismCode)) + 
  facet_wrap(~MethodCode, scales = "free",nrow = 3) + theme(axis.text.x = element_text(angle = 90))

highcatch <- filter(fish_catch_QC, Count> 200) %>%
  filter(!OrganismCode %in% c("SHR", "SHRK")) 

ggplot(highcatch) + geom_boxplot(aes(OrganismCode, Count))
```

QC fish catch/plus counts
```{r}
check_counts <- fish_catch_QC %>%
  left_join(fish_sample_cleaned %>% select(EventID, Datetime, StationCode)) %>%
  filter(PlusCount == TRUE & !is.na(ForkLength)) %>%
  select(EventID, SampleID, Datetime, StationCode, SampleID, OrganismCode, ForkLength, Count, PlusCount)
...
#PlusCountFalse = c(70612, 79900, 79799, 113633, 151311)
PlusCountFalse = c("0100930_1100_AL3_LP10", "20110819_1102_AL4_LP1")

# These had the wrong count
Count1 = c("20110819_1102_AL4_LP1")

# Temporary fix for 20130604_1055_YB_MSS27, which really should be one row plus count, one row FL = 83. Instead, gave another entry with FL = 83 a count of 2. 
Count2 = c("20130604_1055_YB_MSS1")
FLNA = c("20130604_1055_YB_MSS27")
 
# Just some filtering code that was helpful when QCing this section
View(fish_catch_QC %>% filter(EventID == 5464 & OrganismCode == "BGS") %>% select(EventID, SampleID, OrganismCode, ForkLength, Count, PlusCount))



# Make changes -------------------------------------------------
fish_catch_QC2 <- fish_catch_QC %>%
  mutate(PlusCount = replace(PlusCount, SampleID %in% PlusCountFalse, "FALSE"),
         Count = replace(Count, SampleID %in% Count1, 1),
         Count = replace(Count, SampleID %in% Count2, 2),
         ForkLength = replace(ForkLength, SampleID %in% FLNA, NA)
         )
```

## 7. Make final tables

### a) Make table of total catch
```{r}
total_catch <- fish_catch_QC2 %>%
...
### b) Make table for individuals
```{r}
individuals <- fish_catch_QC2 %>%
  filter(PlusCount == FALSE) %>%
  select(FishTagID, SampleID, EventID, OrganismCode, ForkLength, TotalLength, Weight, Sex, RaceByLength, StageCode, Dead, Expression, GeneticSample, FishIDComments)
```

### c) Filter station table to what is in dataset
...
### d) Add in IEP Fish Code and filter to species we have
```{r}
finalFish <- unique(fish_catch_QC2$OrganismCode)

## Keep fixing species
taxonomy_clean <- left_join(fish_tax_clean, iepfishcodes, by = "Taxa") %>%
  filter(OrganismCode %in% finalFish) %>%
  mutate(IEPFishCode = case_when(OrganismCode == "CAT" ~ "UNISIL",
...
Effort table, remove EventIDs that are not in this dataset
```{r}
...
```{r}
write_csv(stations_clean, "data_clean/stations.csv")
write_csv(fish_sample_cleaned, "data_clean/event.csv")
write_csv(effort_final, "data_clean/effort.csv")
write_csv(total_catch, "data_clean/total_catch.csv")
write_csv(individuals, "data_clean/fish_unique")
write_csv(taxonomy_clean, "data_clean/taxonomy.csv")
write_csv(salm_gen_clean, "data_clean/genetics_salmon.csv")
write_csv(smelt_gen_clean, "data_clean/genetics_smelt.csv")
```
