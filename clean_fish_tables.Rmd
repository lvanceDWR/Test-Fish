...
fish_catch_clean <- left_join(fish_catch_clean0, fish_ref_clean %>% select(EventID, Datetime, StationCode)) %>%
  ...
  ungroup() %>%
  select( -Datetime, -StationCode, -num, -Year)
...
smelt_gen_clean <- smelt_gen %>%
  ...
  select(GeneticRowID, FishGenID, qDSM, qWAG, qLFS, GeneticID, Comments_Smelt) 
...
salm_gen_clean <- salm_gen %>%
  ...
  select(GeneticRowID, FishGenID, GeneticID, Probability1 = Prob1, GeneticID2, Probability2 = Prob2, Comments_Salmon, Comments_SalmonDataUsage)
...
individuals0 <- fish_catch_QC3 %>%
  ...
  select(OrganismID, FishGenID, SampleID, EventID, OrganismCode, ForkLength, TotalLength, Weight, Sex, RaceByLength, StageCode, Dead, Expression, GeneticSample, FishIDComments)
```

Check out small fish
```{r}
...
```{r}
ggplot(individuals) + geom_boxplot(aes(x = OrganismCode, y = ForkLength, fill = OrganismCode)) + 
  facet_wrap(~MethodCode, scales = "free",nrow = 3) + theme(axis.text.x = element_text(angle = 90))

ggplot(total_catch) + geom_boxplot(aes(x = OrganismCode, y = Count, fill = OrganismCode)) + 
  facet_wrap(~MethodCode, scales = "free",nrow = 3) + theme(axis.text.x = element_text(angle = 90))

highcatch <- filter(total_catch, Count> 200) %>%
